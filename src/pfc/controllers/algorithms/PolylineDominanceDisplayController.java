...
import pfc.models.algorithms.polylinedominance.PolylineDominanceAlgorithm;
import pfc.models.algorithms.polylinedominance.PolylineDominanceAlgorithmHistory;
...
public class PolylineDominanceDisplayController extends JPanel implements ExecutionControlPanelActions
{	
	/**
	 * Clase para el componente del grafo reducido
	 */
	private class ViewportReduced extends JComponent
	{
		private GraphSettings 		settings;
		private AffineTransform 	transform;
		private boolean				mouseWheelMoved;
		
		public ViewportReduced ( )
		{
			super( );
			...
			this.addMouseWheelListener( new MouseWheelListener( )
			{
				...
				{
					TransformUtilities.zoomCenter( ViewportReduced.this.transform,
							new Point2D.Double( e.getX( ), e.getY( ) ),
							1 - e.getWheelRotation( ) * UserSettings.instance.scrollIncrementZoom.get( ) );
					ViewportReduced.this.mouseWheelMoved = true;
					ViewportReduced.this.update( );
				}
			} );
		}
		...
		{			
			...
			if ( this.mouseWheelMoved == false )
				TransformUtilities.zoomFit( this.transform, GeometryUtilities.getBounds( reduced ), this.getBounds( ) );
			else
				this.mouseWheelMoved = false;
			...
			// Paint the graph
			GraphDisplayView.paint( g2D, reduced, this.settings );
		}
	 ...
	 */
	private PolylineDominanceAlgorithm						algorithm;
	...
	/** atributos del panel del grafo reduced */
	private JPanel 											viewportReducedPanel;
	private ViewportReduced									viewportReduced;
	/** atributos del panel de dominance */
	...
	private SnapshotList<PolylineDominanceAlgorithmHistory>	history;
	 ...
	 * @param graph Graph
	 */
	public PolylineDominanceDisplayController( Graph graph )
	{				
		this.reduced = graph;
		
		this.algorithm = new PolylineDominanceAlgorithm( graph );
		this.algorithm.addObserver( new Observer( )
		{
			@Override
			public void update(Observable o, Object arg)
			{
				PolylineDominanceDisplayController.this.onAlgorithmChanged( arg );
			}
		} );
		...
		/* Agregar al panel principal el panel del grafo reduced */
		this.setViewportReducedPanel( );
        		...
        		.addGroup( GroupLayout.Alignment.TRAILING, this.algorithmDCLayout.createSequentialGroup( )
        				...
        				.addGroup( this.algorithmDCLayout.createParallelGroup(GroupLayout.Alignment.TRAILING )
        						...
        						.addGroup( this.algorithmDCLayout.createSequentialGroup( )
        								.addComponent( viewportReducedPanel, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE )
	...
	private void setViewportReducedPanel ( )
	{
		this.viewportReducedPanel = new JPanel( new BorderLayout( ) )
		{
			{
				TitledBorder title = BorderFactory.createTitledBorder( new BevelBorder( BevelBorder.LOWERED ) );
				title.setTitle( StringBundle.get( "polyline_dominance_reduced_graph_title" ) );
				title.setTitleJustification( TitledBorder.CENTER );
				this.setBorder( title );
			}
		};
		this.viewportReducedPanel.setBackground( UserSettings.instance.graphBackground.get() );
		this.viewportReducedPanel.setOpaque( true );
		
		this.viewportReduced = new ViewportReduced( );
		this.viewportReducedPanel.add( viewportReduced, BorderLayout.CENTER );
	}
	...
	private void setViewportDominancePanel ( )
	{
		this.viewportDominancePanel = new JPanel( new BorderLayout( ) )
		{
			{
				TitledBorder title = BorderFactory.createTitledBorder( new BevelBorder( BevelBorder.LOWERED ) );
				title.setTitle( StringBundle.get( "polyline_dominance_diagram_title" ) );
				title.setTitleJustification( TitledBorder.CENTER );
				this.setBorder( title );
			}
		};
	...
	{
		PolylineDominanceAlgorithmHistory temp = new PolylineDominanceAlgorithmHistory( null, this.reduced.toString( ), "" );
		this.history = new SnapshotList<PolylineDominanceAlgorithmHistory>( temp );
	...
	{
		String drawing = ( this.algorithm.getDominanceDrawing( ) == null
				? null : this.algorithm.getDominanceDrawing( ).toString( ) );
		String reduced = this.algorithm.getReduced( ).toString( );
		String explanation = this.algorithm.getAlgorithmExplanation( );
		
		this.history.add( new PolylineDominanceAlgorithmHistory( drawing, reduced, explanation ) );	
	}
	...
	{
		PolylineDominanceAlgorithmHistory step = this.history.current( );
		
		this.setReduced( step.getReducedGraph( ) != null ? new Graph( step.getReducedGraph( ) ) : null );
		this.setDrawing( step.getDrawing( ) != null ? new DominanceDrawing( step.getDrawing( ) ) : null );
	 ...
	 */
	public void setReduced( Graph reduced )
	{
