/**
 * ConstrainedVisRepDisplayController.java
 */
...
import pfc.views.display.algorithms.DualPathGraphDisplayView;
...
public class ConstrainedVisRepDisplayController extends JPanel implements ExecutionControlPanelActions
{	
	/**
	...
	private class ViewportDual extends JComponent
	{
		...
		{			
			...
			if ( this.isDrawable( ) )
//				graphBounds = GeometryUtilities.getBounds( dual );
...
//			else
			...
			if ( this.isDrawable( ) )
				DualPathGraphDisplayView.paint( g2D, dual, this.settings );
		}
	 ...
	 */
	private ConstrainedVisRepAlgorithm				algorithm;
	/**
	 ...
	 * @param graph Graph
	 */
	public ConstrainedVisRepDisplayController( Graph graph, NonIntersectingPathList paths )
	{
		this.graph = new Graph( graph.toString( ) );
		
		this.algorithm = new ConstrainedVisRepAlgorithm( this.graph, paths );
		this.algorithm.addObserver( new Observer( )
		{
			@Override
			public void update(Observable o, Object arg)
			{
				ConstrainedVisRepDisplayController.this.onAlgorithmChanged( arg );
			}
		} );
	 ...
	 * de Representacion de Visibilidad
	 */
		...
		{
			{
				...
				title.setTitle( StringBundle.get( "constrained_vis_rep_dual_graph_title" ) );
	...
	private void setViewportVRPanel ( )
	{
		this.viewportVRPanel = new JPanel( new BorderLayout( ) )
		{
			{
				TitledBorder title = BorderFactory.createTitledBorder( new BevelBorder( BevelBorder.LOWERED ) );
				title.setTitle( StringBundle.get( "constrained_vis_rep_diagram_title" ) );
				title.setTitleJustification( TitledBorder.CENTER );
				this.setBorder( title );
			}
		};
