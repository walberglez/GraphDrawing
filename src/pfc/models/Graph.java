...
public class Graph extends ObservableModel
{
	...
	/**
	 * A {@code Integer} number that indicates the new {@code Vertex}'s label available
	 */
	private Integer							newLabelAvailable = 0;
	...
	public Graph( Map<String, Object> attributes )
	{
		...
		for( Object vertex : (Iterable<?>) attributes.get( "vertices" ) )
			if( vertex instanceof Map<?, ?> )
			{
				...
				// set new default vertex's Label
//				this.setNewVertexLabelAvailable( newVertex );
	...
	public Set<Edge> getEdges( Vertex from, Vertex to )
	{
		...
		for( Edge edge : this.incidences.get( from ) )
			if( ( edge.from.equals( from ) && edge.to.equals( to ) ) ||
					( !edge.isDirected && edge.from.equals( to ) && edge.to.equals( from ) ) )
	 ...
	 * 
	 * @param vertex {@link Vertex} the vertex to which the returned vertices are neighbors
	...
	public Set<Vertex> getNeighbors( Vertex vertex )
	{
		...
		for( Edge edge : this.getEdgesFrom( vertex ) )
			neighbors.add( ( edge.from.equals( vertex ) ) ? edge.to : edge.from );
		
		return neighbors;
	}
	
	/**
	 * Returns set of neighbors B of A if there exists and edge from B to A, regardless of how
	 * many edges exist from A to B.
	 * 
	 * @param vertex {@link Vertex} the vertex to which the returned vertices are neighbors
	 * @return the neighbors of the specified vertex
	 ...
	 * @see Edge
	 */
	public Set<Vertex> getNeighborsIn( Vertex vertex )
	{
		Set<Edge> in = this.getEdgesTo( vertex );
	...
	/**
	 * Returns set of neighbors B of A if there exists and edge from A to B, regardless of how
	 * many edges exist from B to A.
	 * 
	 * @param vertex {@link Vertex} the vertex to which the returned vertices are neighbors
	 * @return the neighbors of the specified vertex
	 * @see Vertex
	 * @see Edge
	 */
	public Set<Vertex> getNeighborsOut( Vertex vertex )
	{
		Set<Edge> out = this.getEdgesFrom( vertex );
	...
	/**
	 * Obtener todos los vertices vecinos del vertice v tanto salientes como entrantes.
	 * 
	 * @param vertex {@link Vertex}
	 * @return Set<Vertex>
	 */
	public Set<Vertex> getNeighborsInOut( Vertex vertex )
	{
		Set<Vertex> neighbors = new HashSet<Vertex>( this.getNeighborsIn( vertex ) );
		neighbors.addAll( this.getNeighborsOut( vertex ) );
		return neighbors;
	}
	...
	/**
	 * Asignar la nueva etiqueta disponible al vertice.
	 * @param newVertex Vertex
	 */
	public void setNewVertexLabelAvailable ( Vertex newVertex )
	{
		// set new default vertex's Label
		newVertex.label.set( newLabelAvailable.toString() );
		newLabelAvailable++;
	}
	
	/**
	 * @return String nueva etiqueta de vertice disponible.
	 */
	public String getNewLabelAvailable ( )
	{
		return "" + this.newLabelAvailable++;
	}
	

	/**
	 * Precondition: graph es un st-graph
	 * @return {@link Vertex} vertice Source del st-graph G
	 */
	public Vertex getVertexSource( )
	{
		if ( areDirectedEdgesAllowed )
		{
	...
	/**
	 * Precondition: graph es un st-graph
	 * @return {@link Vertex} vertice Target del st-graph G
	 */
	...
	{
		return ( ( this.getEdgesFrom( v ).isEmpty() == false )
				&& ( this.getEdgesTo( v ).isEmpty() ) );
	}
	...
	{
		return ( ( this.getEdgesTo( v ).isEmpty() == false )
				&& ( this.getEdgesFrom( v ).isEmpty() ) );
	}
}
