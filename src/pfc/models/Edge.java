...
import static pfc.utilities.GeometryUtilities.*;
...
public class Edge extends ObservableModel
{
	...
	/**
	 * A {@code List} of {@code Point2D} bends
	 */
	private List<Point2D>	bends;
	...
	public Edge( boolean isDirected, Vertex from, Vertex to, double weight, int color, String label, boolean isSelected )
	{
		...
		this.isLoop = ( this.from.equals( this.to ) );
		...
		this.tag = new Property<String>( "" );

		this.arc = new Arc2D.Double( );
		this.line = new Line2D.Double( );
		this.center = new Point2D.Double( );

		this.bends = new ArrayList<Point2D>( );
	...
	public Edge( Map<String, Object> members, Map<String, Vertex> vertices )
	{
		...
		this.tag = new Property<String>( members.containsKey( "tag" ) ? (String) members.get( "tag" ) : "" );
		...
		this.bends = new ArrayList<Point2D>( );
		if ( members.containsKey( "bends" ) )
		{
			for( Object bend : (Iterable<?>) members.get( "bends" ) )
				if( bend instanceof Map<?, ?> )
				{
					@SuppressWarnings("unchecked")
					Map<String, Object> point = (Map<String, Object>) bend;
					this.bends.add( new Point2D.Double( ( Double ) point.get( "x" ), ( Double ) point.get( "y" ) ) );
				}
		}
	...
	/**
	 * Returns this {@code Edge}'s bends set.
	 * 
	 * @return bends List<Point2D>
	 */
	...
	/**
	 * Returns the total of bends of this {@code Edge}
	 * 
	 * @return int
	 */
	public int getTotalOfBends( )
	{
		Set<Point2D> bends = new HashSet<Point2D>( this.bends );
		int total = 0;
		for ( Point2D p : bends )
		{
			if ( p.getX( ) != this.from.x.get( ) || p.getX( ) != to.x.get( ) )
				total++;
		}
		return total;
	}
	 ...
	 */
	public void setBends( List<Point2D> bends )
	{
		this.bends = bends;
	}
	...
	{
		return this.isLinear && isPolyline( ) == false;
	}
	
	/**
	 * Returns a {@code boolean} indicating whether or not the this {@code Edge} is to be treated as a polyline segment between its two vertices.
	 * 
	 * @return {@code true} if this {@code Edge} is polyline, {@code false} otherwise
	 */
	public boolean isPolyline( )
	{
		return this.bends != null && this.bends.isEmpty( ) == false;
	}
	...
	public int hashCode( )
	{
		final int prime = 31;
		int result = 1;
		result += prime * ( ( this.from == null ) ? 0 : this.from.hashCode( ) );
		result += prime * ( ( this.to == null ) ? 0 : this.to.hashCode( ) );
		return result;
	}
