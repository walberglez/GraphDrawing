 ...
 * @navassoc - - "-visibility" VisibilityRepresentationAlgorithm
 * @depend - - - Edge
 * @depend - - - Vertex
 * @depend - - - EdgeSegment
 ...
 * @depend - - - StringBundle
 */
public class UpwardPolylineAlgorithm extends ObservableModel
{
	/**
	...
	/**
	 * Representacion de Visibilidad
	 */
	private VisibilityRepresentationAlgorithm	visibility;
	/**
	 * Polyline graph
	 */
	 ...
	 */
	public UpwardPolylineAlgorithm( Graph stGraphG )
	{
		this.visibility = new VisibilityRepresentationAlgorithm( stGraphG );
		...
		this.elementObserver = new Observer( )
		{
			...
			{
				UpwardPolylineAlgorithm.this.setChanged( );
				UpwardPolylineAlgorithm.this.notifyObservers( );
			}
		};
	...
	public String getAlgorithmExplanation ( )
	{
		...
		{
		case VISIBILITY:
			algorithmExplanation = StringBundle.get( "upward_polyline_algorithm_step_visibility" );
			break;
		case HORIZONTAL_VERTICES:
			algorithmExplanation = StringBundle.get( "upward_polyline_algorithm_step_horizontal_vertices" );
			break;
		case VERTICAL_EDGES:
			algorithmExplanation = StringBundle.get( "upward_polyline_algorithm_step_vertical_edges" );
			break;
		}
	...
	/**
	 * Ejecuta las distintas etapas del algoritmo Upward Polyline
	 * descrito en el libro {@link "http://www.cs.brown.edu/~rt/gdbook.html"}, pag 107.
	 */
	...
	private void constructVerticalEdges ( )
	{
		...
		for ( EdgeSegment eS : getVisibilityDrawing( ).edgeSegments )
		{
			...
			e = new Edge(
					polyline.areDirectedEdgesAllowed,
					polyline.vertices.get( indFrom ),
					polyline.vertices.get( indTo ) );
			
			if ( eS.isShortEdge( ) == false )
			{
				bends = new ArrayList<Point2D>( );
				// (x(T(u,v),y(u)+1)
				bends.add( getBendFromPositioning( eS ) );
				// (x(T(u,v),y(v)-1)
				bends.add( getBendToPositioning( eS ) );
				e.setBends( bends );
			}
